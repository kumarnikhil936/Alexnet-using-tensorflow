...
tf.app.flags.DEFINE_string('data_dir', '/tmp/cifar10_data', """Path to the CIFAR10 directory.""")
...
# If a model is trained with multiple GPUs, prefix all Op names with tower_name to differentiate with other operations.
...
def _variable_with_weight_decay(name, shape, stddev, wd):
    ...
    Args:
      ...
      wd: add L"Loss weight decay multiplied by this float.
...
def inference(images):
    ...
    # in order to share variables across multiple GPU training runs.
    ...
    # linear layer (WX + b), We dont apply softmax here because tf.nn.sparse_softmax_cross_entropy_with_logits
...
def loss(logits, labels):
    ...
    cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=labels, logits=logits,
                                                                   name='cross_entropy_with_example')
...
def _add_loss_summaries(total_loss):
    ...
    Add summaries for losses in CIFAR 10 model.
      Generates moving average for all losses and associated summaries for visualizing the performance in the network.
...
def train(total_loss, global_step):
    ...
    Create an optimizer and apply to all trainable variables. Add moving average fpr all trainable variables.
